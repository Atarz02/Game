...
$(document).ready(function() {
  ...
  function updateContent() {
    $.ajax({
      url: "update.php", // URL of server-side script to handle data update
      type: "GET", // HTTP request method
      dataType: "html", // Expected data type from the server
      success: function(response) {
        $("#content").html(response); // Update content with response from server
      },
  ...
  // Call updateContent function initially and every 5 seconds
  updateContent();
  setInterval(updateContent, 5000); // Update every 5 seconds (5000 milliseconds)
});
